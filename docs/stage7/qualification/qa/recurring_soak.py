...
path=OUT/'qualified-recurring-grant.json';assert not path.exists(),'Fresh elapsed acceptance only'
s,grant,_=http(APP,'/api/recurring','POST',{'operation':'answer','question':'What does RFC 2606 say about example domain names?','maxRuns':8,'minIntervalSeconds':180,'expiresInSeconds':7200},headers);assert s==201,(s,grant)
path.write_text(json.dumps(grant));path.chmod(0o600)
spec={k:grant[k] for k in ('grantId','token')};spec.update(intervalSeconds=180,timezone='Europe/Madrid',maxRuns=8,estimatedUnitMicros=1000,maxEstimatedMicros=8000,noProgressSeconds=1200)
start_wall=time.time();start_mono=time.monotonic();m=native('create',spec);id=m['id']
(OUT/'qualified-recurring-start.json').write_text(json.dumps({'id':id,'startedAt':start_wall,'cronId':m['cronId']}))
print(json.dumps({'nativeCronCreated':True,'id':id,'cronId':m['cronId'],'realElapsedRunStarted':True}),flush=True)
restarted=False;version=-1;deadline=time.monotonic()+7200
while time.monotonic()<deadline:
 ...
 if m['active'] and not restarted:
  ...
  if state['request'] and state['request']['state']=='result-ready':
   ...
   restarted=True;(OUT/'qualified-recurring-restart.json').write_text(json.dumps({'nativeRestartPreservedCheckpoint':True,'checkpoint':checkpoint,'afterVersion':recovered['version']}))
...
completion_elapsed=time.monotonic()-start_mono
while time.monotonic()-start_mono<1800:
 terminal=native('status',{'id':id});assert terminal==m,'Terminal workflow changed during durability observation'
 time.sleep(min(5,1800-(time.monotonic()-start_mono)))
elapsed=time.monotonic()-start_mono
assert elapsed>=1800,'Measured recurring execution was shorter than acceptance window'
assert restarted and len(m['events'])==8 and all(e.get('completedAt') for e in m['events'])
assert len({e['requestId'] for e in m['events']})==8 and len({e['receiptId'] for e in m['events']})==8
ledger=[]
for e in m['events']:
...
(OUT/'qualified-recurring-native-final.json').write_text(json.dumps(m,indent=2));(OUT/'qualified-recurring-core-receipts.json').write_text(json.dumps(ledger,indent=2))
report={'passed':True,'clockOrOutcomeEdits':False,'elapsedSeconds':round(time.monotonic()-start_mono,3),'completionElapsedSeconds':round(completion_elapsed,3),'terminalDurabilityObservationIncluded':True,'wallElapsedSeconds':round(time.time()-start_wall,3),'successfulRecurringResults':8,'uniqueAppRequests':8,'uniqueCoreReceipts':8,'nativeRestartPreservedCheckpoint':restarted,'nativeAuthority':authority,'missedSlotsCoalesced':m['missedCoalesced'],'estimatedReservedMicros':m['estimatedReservedMicros'],'providerCost':m['providerCost'],'providerCostIsMeasured':False,'realModelInference':True,'dayOrWeekSoakClaim':False}
...
(OUT/'qualified-recurring-soak.json').write_text(json.dumps(report,indent=2));print(json.dumps(report),flush=True)
