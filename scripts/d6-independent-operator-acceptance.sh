...
DIND_IP=$(docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' "$DIND");test -n "$DIND_IP"
...
set +e
run "for _ in \$(seq 1 120); do code=\$(curl -ksS -o /dev/null -w '%{http_code}' https://alica-d6.localhost/api/v1/frameworks || true); [ \"\$code\" = 401 ] && exit 0; sleep 2; done; echo final_http_code=\$code; getent hosts alica-d6.localhost; curl -kv --connect-timeout 10 https://alica-d6.localhost/api/v1/frameworks; exit 1" > "$EVIDENCE/readiness.txt" 2>&1
